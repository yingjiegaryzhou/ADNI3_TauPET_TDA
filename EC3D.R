...
#'img.dir' determines the subfolders where the images are located. This can be a pattern.
...
#'first.only' refers to the issue of multiple unconnected shapes or regions in the image (e.g. multifocal tumors with multiple masses). If TRUE, the function will only compute the EC for the 'first' identified shape.
...
ecf <- function(in.dir,out.file,img.dir=NULL,stepsize=100,rotstep=72,first.only=TRUE,verbose=FALSE){
  ...
  #Output #2: Shapes is the resulting structural array where:
  ...
  # Assume that the code is being executed somewhere containing a folder called
  ...
  d= rbind(d1,d2) #2x73 vector if rotstep=72
  ...
  for (k in 1:n.patients) {
    ...
    if ( length(dir(pattern='.png')) > 0 ) {
      slices <- dir(pattern="*.png") # Collection of slices for the kth patient
      EC = matrix(0,stepsize+1,dim(d)[2]-1) # 2d empty matrix with m=stepsize + 1 n=rotstep
      ec.slices <- array(0,dim=c(dim(EC),length(dir(pattern='.png')))) # add in 3rd dimension, which is number of images per individual(22 in our case); 101x72x22 matrix
      realslice = 0
      
      # Compute the EC for all slices of patient k
      # j goes through the images/slices
      for ( j in 1:length(slices) ) {
        if(verbose == TRUE ) print(slices[j])
        I = readPNG(slices[j])[,,1] #For each slice, get 480x480 dimension matrix. 4 slices for example
        # Only use images that have non-zero regions
        if ( sum(I) > 10 ) { #sum values of 2d matrix
          BW = round(I)
          ...
          if ( first.only == TRUE ) {
            ...
            for ( comp.ind in c.unique ) {
              ...
              if(nrow(ones)>5){
                ...
                complex$V=Z
                complex$E=E
                complex$F=NULL #In 3D shapes, there are triangular meshes
                #In imaging case we're not triangulating the black and white greyscale images, hence why F is set as NULL here. 
                #It's all one face bc we have points where edges meet (vertices).
                #Images not triangulated for initial computation of EC.
...
connected.components <- function(BW){
  ...
  lab.mat <- matrix(0,dim(BW)[1],dim(BW)[2]) #empty 480x480 matrix
  new.lab <- 1
  ...
  for ( i in 1:dim(BW)[1] ) {        
    for ( j in 1:dim(BW)[2] ) {
      if (BW[i,j] == 1) {
        ...
        i.range <- 0:2-1 #if any cell in the rounded matrix is 1, its range is -1 to 1 for i and j
        j.range <- 0:2-1
        if ( i==1 ) i.range <- 0:1 #first row the range is [0,1]
        if ( i==dim(BW)[1] ) i.range <- 0:1-1 #last row range is [-1, 0]
        if ( j==1 ) j.range <- 0:1 #first column range is [0, 1]
        if ( j==dim(BW)[2] ) j.range <- 0:1-1 #last col range is [-1, 0]
        #                print(i + i.range)
        #                print(j + j.range)
        current.neighbours <- BW[i+i.range,j+j.range] #get the neighbors around each cell of BW matrix?
        current.neighbours[2,2] <- 0 #Why set to 0?
